...
**Katmanlı parser** (`config.PARSER`: `auto`/`pymupdf`/`docling`/`chandra`): `extract_tree` önce gömülü TOC'u dener, yoksa `_parse_content` ile içerik-tabanlı parser'a gider. `auto`/`docling` modunda **Docling** (`_try_docling`, layout-model, alt-bölüm çözünürlüğü; `pip install docling` gerekir) denenir; kurulu değil/başarısızsa **PyMuPDF sezgisel** yola **fail-safe** düşülür ("16GB'da çalışır" vaadi korunur). Docling, belgeyi Markdown'a çevirip `extract_toc_tree`'ye verir (tüm ağaç mantığı ortak; v1'de sayfa atfı bölüm-seviyesinde).
